...
def create_profile_no_proxy(token):
    conn = http.client.HTTPSConnection("anty-api.com")
    payload = json.dumps({
        "common": {
            "platform": "windows",
...
def Runing_process(token,id,port,wsEndpoint,rep):
    global driver
    try:
    ...
    try:
        ...
        input_class('r-30o5oe',t_pass[rep],0,3,2)
        click('css-901oao','Next',15,3,2)
        print(f'3.2 Вставляем email {h_login[rep]}')
        input_class('r-30o5oe',h_login[rep],0,3,2)
        click('css-901oao','Next',20,3,2)
    except:
        print(f'Ошибка {rep+1} итерации')
        try:
            time.sleep(3)
    ...
    try:
        ...
        '''with open("saveData.json", "a") as outfile:
            ...
            outfile.write(',\n')'''
        with open("saveData.json", "a", encoding='utf-8') as outfile:
            outfile.write('{\n')
...
if __name__ == '__main__':
    ...
    while rep<int(repeat):
        if iter_proxy>=len_proxy:
            iter_proxy=0
        token=get_Dolphin_token()
        time.sleep(3)
        if len_proxy==0:
            id=create_profile_no_proxy(token)
            time.sleep(3)
            try:
                port,wsEndpoint=run_browser(id,'dolphin')
            except:
                print('Error. Delay 180s.')
                time.sleep(180)
                continue
        else:
            ...
            try:
                ...
                print('Proxy error. Delay 180s.')
                iter_proxy+=1
                time.sleep(180)
                continue
        time.sleep(3)
        p = multiprocessing.Process(target=Runing_process, args=(token,id,port,wsEndpoint,rep))
        p.start()
        ...
        if p.is_alive():
            print(f"Время выполнения {rep+1} итерации закончилось")
            try:
                time.sleep(3)
                stop_browser(id,'dolphin')
                time.sleep(5)
                ...
                pass
            with open("error.txt", "a", encoding='utf-8') as out:
                out.write(f"{datetime.now()} - Время выполнения {rep+1} итерации закончилось\n")
                out.write(f"Email:{h_login[rep]}:{h_pass[rep]}\n")
                out.write(f"Twitter:{t_login[rep]}:{t_pass[rep]}:{t_phone[rep]}\n")
                out.write("-------------------------------------------------------------------\n")

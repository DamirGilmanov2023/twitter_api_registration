...
twitter_file.close()
print(t_login,t_pass,t_phone)'''

twitter_file=open('твиттер2.txt','r')
twitter_f=twitter_file.read().split('\n')
twitter=[]
for t in twitter_f:
    if t!="":
...
for t in twitter:
    a=t.split(':')
    t_login.append(a[1].replace(' ',''))
    t_pass.append(a[2].replace(' ',''))
    t_phone.append(a[3].replace(' ',''))
    t_auth_token.append(a[4].split('=')[1].replace(' ',''))
twitter_file.close()
#for i in range(len(twitter)):
#    print(t_login[i],t_pass[i],t_phone[i],t_auth_token[i])
#print(t_login,t_pass,t_phone,t_auth_token)
...
def click(class_,name_class,i,time_before,time_after):
    ...
    if tmr():
        ...
        if check_exists_class(class_,name_class,i) and tmr():
...
def input_id(id_,keys,time_before,time_after):
    ...
    time.sleep(time_before)
    iter_error=0
    while tmr() and driver.find_element(By.ID,id_).get_attribute('value')!=keys:
        try:
...
def input_class(class_,keys,i,time_before,time_after):
    ...
    time.sleep(time_before)
    iter_error=0
    while tmr() and driver.find_elements(By.CLASS_NAME,class_)[i].get_attribute('value')!=keys:
        time.sleep(1)
        try:
...
for rep in range(int(repeat)):
    ...
    try:
        if tmr():
            ...
            print(f'Время {rep+1} итерации вышло')
            driver.quit()
            continue
    except:
    ...
    try:
        if tmr():
            ...
            print(f'Время {rep+1} итерации вышло или аккаунт заблокирован')
            driver.quit()
            continue
    except:
        print(f'Ошибка {rep+1} итерации')
    ...
    try:
        if tmr():
            ...
            while flag and tmr():
                ...
                click_one_click('Button-label','Let\'s do this',0,1,5)
                mmm=''
                try:
